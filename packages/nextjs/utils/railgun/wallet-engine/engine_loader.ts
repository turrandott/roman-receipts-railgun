...
const loadEngineProvider = async () => {
  ...
  const GOERLI_PROVIDERS_JSON: FallbackProviderJsonConfig = {
    chainId: 5,
    providers: [
      {
  ...
  // Loading providers and setting fees for Goerli Testnet
  const { feesSerialized } = await loadProvider(GOERLI_PROVIDERS_JSON, NetworkName.EthereumGoerli, shouldDebug);
  setRailgunFees(NetworkName.EthereumGoerli, BigInt(feesSerialized.shield), BigInt(feesSerialized.unshield));
