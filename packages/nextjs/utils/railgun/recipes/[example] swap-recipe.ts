...
import { compareERC20Info, getIsUnvalidatedRailgunAddress } from "@railgun-community/cookbook/dist/utils";
...
export abstract class SwapRecipe extends Recipe {
  ...
  protected abstract getSwapQuote(networkName: NetworkName, sellERC20Amount: RecipeERC20Amount): Promise<SwapQuoteData>;

  getBuySellAmountsFromRecipeOutput(recipeOutput: Optional<RecipeOutput>): Optional<{
    ...
    try {
      ...
      if (isDefined(this.destinationAddress) && !getIsUnvalidatedRailgunAddress(this.destinationAddress)) {
