import { ERC20Contract } from "@railgun-community/cookbook/dist/contract/token/erc20-contract";
import {
  RecipeERC20Info,
  StepConfig,
  StepInput,
...
} from "@railgun-community/cookbook/dist/models/export-models";
import { Step } from "@railgun-community/cookbook/dist/steps/step";
import { maxBigNumberForTransaction, minBigNumber } from "@railgun-community/cookbook/dist/utils/big-number";
import { createNoActionStepOutput } from "@railgun-community/cookbook/dist/utils/no-action-output";
import { compareERC20Info } from "@railgun-community/cookbook/dist/utils/token";
...
export class ApproveERC20SpenderStep extends Step {
  ...
  constructor(spender: Optional<string>, tokenInfo: RecipeERC20Info, amount?: bigint) {
    super();
  ...
  protected async getStepOutput(input: StepInput): Promise<UnvalidatedStepOutput> {
    ...
    const { erc20AmountForStep, unusedERC20Amounts } = this.getValidInputERC20Amount(
      erc20Amounts,
      erc20Amount => compareERC20Info(erc20Amount, this.tokenInfo) && erc20Amount.approvedSpender !== this.spender,
      this.amount,
    );
    ...
    if (this.requiresClearApprovalTransaction(networkName, erc20AmountForStep.tokenAddress)) {
      crossContractCalls.push(await contract.createSpenderApproval(this.spender, 0n));
    }
   ...
   */
  private requiresClearApprovalTransaction(networkName: NetworkName, tokenAddress: string): boolean {
    switch (networkName) {
